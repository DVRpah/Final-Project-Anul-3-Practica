...
function Dashboard() {
  ...
  const [newPet, setNewPet] = useState({ name: '', species: 'caine', breed: '', age: '', weight: '' })
  ...
  const handleAddPet = async (e) => {
    ...
    try {
      ...
      setShowAddPet(false)
      setNewPet({ name: '', species: 'caine', breed: '', age: '', weight: '' })
      fetchData()
    } catch (err) {
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 min-h-screen p-4 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-800 dark:text-white">Bună ziua, {user.firstName}! 👋</h1>
              ...
              {loading ? (
                ...
                upcomingAppointments.map(app => (
                  <div key={app.id} className="flex items-center gap-4 p-3 bg-emerald-50 dark:bg-gray-700 rounded-xl">
                    ...
                    <div>
                      <p className="font-semibold text-gray-800 dark:text-white">{app.service?.name} — {app.pet?.name}</p>
                      <p className="text-gray-500 dark:text-gray-400 text-sm">{new Date(app.date).toLocaleDateString('ro-RO')}</p>
                      <span className="text-xs bg-emerald-100 text-emerald-600 px-2 py-0.5 rounded-full">{app.status}</span>
                    </div>
                  </div>
                ))
              )}
      ...
      {showAddPet && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-gray-800 dark:text-white">Adaugă animal</h3>
            ...
            <form onSubmit={handleAddPet} className="space-y-4">
              ...
              <div>
                <label className="block text-gray-700 dark:text-gray-300 font-medium mb-1">Specie</label>
              ...
              <div>
                <label className="block text-gray-700 dark:text-gray-300 font-medium mb-1">Rasă</label>
                <input type="text" value={newPet.breed} onChange={e => setNewPet({...newPet, breed: e.target.value})}
                  className="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-xl px-4 py-2 focus:outline-none focus:border-emerald-500" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-gray-700 dark:text-gray-300 font-medium mb-1">Vârstă (ani)</label>
                  <input type="number" value={newPet.age} onChange={e => setNewPet({...newPet, age: e.target.value})}
                    className="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-xl px-4 py-2 focus:outline-none focus:border-emerald-500" />
                </div>
                <div>
                  <label className="block text-gray-700 dark:text-gray-300 font-medium mb-1">Greutate (kg)</label>
              ...
              <button type="submit" disabled={savingPet}
                ...
                {savingPet ? 'Se salvează...' : 'Adaugă animal'}
              </button>
            </form>
          </div>
        </div>
